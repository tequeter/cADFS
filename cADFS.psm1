...
[DscResource()]
class cADFSWebApplicationProxy {
    <#
    The Ensure property is used to determine if the Active Directory Federation Service (ADFS) should be installed (Present) or not installed (Absent).
    #>
    [DscProperty(Mandatory)]
    ...
    [cADFSWebApplicationProxy] Get() {

        Write-Verbose -Message 'Starting retrieving Web Application Proxy App.';

        try {
            $WebApplicationProxyApplication = Get-WebApplicationProxyApplication -Name $this.Name -ErrorAction Stop;
        }
        catch {
            Write-Verbose -Message ('Error occurred while retrieving Web Application Proxy App: {0}' -f $global:Error[0].Exception.Message);
        }

        Write-Verbose -Message 'Finished retrieving a Web Application Proxy App.';
        return $this;
    }
    ...
    [System.Boolean] Test() {
        ...
        Write-Verbose -Message 'Testing for presence of a Web Application Proxy App.';
        ...
        if ($this.Ensure -eq 'Present') {
            Write-Verbose -Message 'Checking for presence of a Web Application Proxy App.';
        }

        if ($this.Ensure -eq 'Absent') {
            Write-Verbose -Message 'Checking for absence of a Web Application Proxy App.';
            if ($WebApplicationProxyApplication) {
                Write-Verbose -Message
                $Compliant = $false;
            }
        }
    ...
    [void] Set() {
        ...
        if ($this.Ensure -eq [Ensure]::Present) {
                ...
                $Proxy = @{
                    FederationServiceTrustCredential = $this.ServiceCredential;
                    CertificateThumbprint = $this.CertificateThumbprint;
                    FederationServiceName = $this.FederationName;
                };

                Install-WebApplicationProxy @Proxy
                
                Write-Verbose -Message 'Adding Web Application Proxy Application.';
                $FederationURL = "https://$($this.FederationName)";
                $App = @{
                    Name = $this.Name;
                    BackendServerUrl = $FederationURL;
                    ExternalUrl = $FederationURL;
                    ExternalCertificateThumbprint = $this.CertificateThumbprint;
                    ExternalPreAuthentication = $this.ExternalPreAuthentication;
                };                
                Add-WebApplicationProxyApplication @App
        }

        if ($this.Ensure -eq [Ensure]::Absent) {
            Write-Verbose -Message 'Removing Web Application Proxy Application.';
            Remove-WebApplicationProxyApplication -Name $this.Name
        }
